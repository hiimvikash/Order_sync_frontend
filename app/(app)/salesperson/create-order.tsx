...
const CreateOrder = () => {
  ...
  >({});
  const [itemModalStep, setItemModalStep] = useState<
    "category" | "product" | "variant" | "quantity"
  >("product");
  ...
  const fetchData = async () => {
    try {
      ...
      const [categoryRes, productsRes, shopkeepersRes, distributorsRes] =
        await Promise.all([
          axios.get(`${baseURL}/salesperson/get-category`, { headers }),
          axios.get(`${baseURL}/salesperson/get-products`, { headers }),
          axios.get(`${baseURL}/salesperson/${parsedUser.id}/shops`, {
            headers,
          }),
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!formData.shopkeeperId)
      newErrors.shopkeeperId = "Shopkeeper is required";
    if (!formData.distributorId)
      newErrors.distributorId = "Distributor is required";
    if (!formData.deliveryDate)
      newErrors.deliveryDate = "Delivery date is required";
    if (!formData.deliverySlot)
      newErrors.deliverySlot = "Delivery slot is required";
    ...
    if (formData.paymentTerm === "PARTIAL") {
      if (!formData.partialPayment) {
        ...
        const { initialAmount, remainingAmount, dueDate } =
        ...
        if (initialAmount + remainingAmount !== formData.totalAmount) {
          ...
          newErrors.partialPayment =
            "Initial and remaining amounts must equal total amount";
        }
  ...
  const handleInputChange = useCallback(
    (field: keyof OrderFormData, value: any) => {
      setFormData((prev) => ({
        ...prev,
        [field]:
          field === "shopkeeperId" || field === "distributorId"
            ? Number(value)
            : value,
      }));
    },
  ...
  const handlePartialPaymentChange = useCallback(
    (field: keyof PartialPayment, value: any) => {
      setFormData((prev) => ({
        ...prev,
        partialPayment: {
          ...prev.partialPayment!,
    ...
    try {
      ...
      if (formData.paymentTerm === "PARTIAL") {
        ...
        if (partialPayment) {
          ...
          Alert.alert(
            "Validation Error",
            "Partial payment details are required."
          );
          return; // Prevent order creation
        }
      }
          ...
          <View style={styles.modalContent}>
            ...
            >
              ...
              <Text>
                Shopkeeper:{" "}
                {
                  shopkeepers.find((shop) => shop.id === formData.shopkeeperId)
                    ?.name
                }
              </Text>
              <Text>
                Distributor:{" "}
                {
                  distributors.find((d) => d.id === formData.distributorId)
                    ?.name
                }
              </Text>
    ...
    return (
      <View style={styles.partialPaymentSection}>
        ...
        <TextInput
          ...
          onChangeText={(value) =>
            handlePartialPaymentChange("initialAmount", value)
          }
        />
        ...
        <TextInput
          ...
          onChangeText={(value) =>
            handlePartialPaymentChange("remainingAmount", value)
          }
        />
        ...
        {showDueDatePicker && (
          <DateTimePicker
            value={
              formData.partialPayment?.dueDate
                ? new Date(formData.partialPayment.dueDate)
                : new Date()
            }
            mode="date"
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        />
        {errors.shopkeeperId && (
          <Text style={styles.errorText}>{errors.shopkeeperId}</Text>
        )}
        ...
        {/* <View style={[styles.pickerContainer]}>
          <Picker
            ...
            onValueChange={(value) =>
              handleInputChange("distributorId", String(value))
            }
          >
...
<SearchableDropdown
          ...
          onSelect={(value) => handleInputChange("distributorId", String(value))}
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {itemModalStep === "product" && (
              <FlatList
                data={products.filter((p) => p.categoryId === Number(selectedCategory))}
                keyExtractor={(item) => item.id}
                renderItem={({ item }) => (
                  ...
                  >
                    ...
                    >
                      <View>
                        {(() => {
                          const [firstLine, secondLine] = formatProductName(
                            item.name,
                            22
                          );
                          return (
              ...
              <View style={styles.quantityContainer}>
                ...
                <View style={styles.quantityControls}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(value) =>
                      setQuantity(Math.max(1, parseInt(value) || 0))
                    }
                    keyboardType="numeric"
                  />
                  <TouchableOpacity
